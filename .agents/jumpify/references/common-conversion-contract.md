...
When one entered identity is priced or classified by several tiers, model the direct scalar identity input once and the tier selection separately. When tiers classify the entity, put the complete classification set—and no unrelated member—in one single-select Source; every member writes an explicit source-authored value to the same Property, and the entity's visible Tracker name interpolates the selected value. A classification Choice cannot own or repeat the scalar input. All tier and upgrade effects must target the same stable grant when Format 1 provides the required reference or interpolation path. Do not create a separate companion merely because each tier has its own Choice. Effects explicitly belonging to a Companion use Companion ownership; effects explicitly belonging to a Form use Form ownership.

If mutually exclusive Choices classify what that one entity is, record their one single-select Source and complete membership on the dynamic entity. Each Choice grants the same stable Property with its own explicit source-authored value, and the entity's Tracker name interpolates the selected Property value. A selectable but grantless classification or an omitted value that copies a boolean toggle discards the User's selected meaning. This is contextual: a similarly named Choice that only adds an ability or enhancement remains a targeted Perk rather than a Property, while a Choice that both classifies and explicitly enhances the entity grants both the Property and targeted Perk.

Do not turn incidental narrative people, creatures, possessions, or scenery into durable Tracker grants unless the source clearly grants them mechanically. Resolve repeated mentions, pronouns, and later defining Choices before deciding that a narrative entity is new. Quote the ambiguous mention with contiguous relationship context, then verify every same/new target is an existing canonical Choice and the grant actually created by that Choice. Permission to keep something already owned is not an authored new Item grant. Give every distinct clause its own semantic force, exact source evidence, dispositions, and reason; never assign one force to a paragraph that mixes retained objects with explicit new grants. Only explicit-grant/current-Jump-rule clauses may own unconditional grant keys. Use continuity semantics for an unchanged previous identity when the source makes continuity free; do not replace it with a charged ordinary option.
...
Tags describe effects. Use one to five concrete existing built-ins as appropriate; add a narrow custom Tag only when it improves User filtering. Give every visible Perk, Item, Companion, Form, and Trait grant effect Tags as well as each taggable Choice. Choose the count from useful independent filtering dimensions and record why each Tag applies. Do not Tag costs, discounts, prerequisites, sections, or redundant grant kinds. A substantial conversion where every grant or Choice receives exactly one Tag is presumptively an unreviewed bulk assignment, not a semantic classification. If all affected entries genuinely have only one useful dimension, record a justified `tagCardinalityReview` whose exact Choice-handle and owner/index-qualified grant-ref inventories reconcile with the complete all-one sets, including duplicate-named grants; otherwise vary Tags according to effect rather than padding a quota.
