...
Traits are Jump-local and must not be modeled as chain-wide perks or items. If the source grants a named companion from entered text, interpolate that answer into the visible companion name and verify it in the Companions tab.
...
For `resolution: either`, preserve ordinary authored costs. The recorded rolled Choice or scalar value receives the native free roll allowance, while a manual result uses the normal cost. Verify both Cost states in Tracker before reporting a pricing gap.

Inspect and screenshot selected, unselected, disabled-at-limit, rolled, manual, discounted, repeated-rank, and negative-cost states. The same Playwright action must assert its resulting value and visible price, record DOM observations, and capture Section-relative card/rail/neighbor bounds; a file named `changed.png` is not proof that anything changed. Compare bounds before and after interaction and require empty live-element overlap findings at 1440px, 720px, and 390px. A pick limit must disable only additional selections; it must not resize or stretch remaining controls. Selecting a primary value must not reveal that value's real control behind a generic activation checkbox/radio.

## Assets and crops

Use meaningful alt text describing the relevant image, not its filename or coordinates. Record every crop's source page, rectangle, purpose, mode, and edge ownership.

Run `crop-assets.mjs` from ledger coordinates. Inspect all four edges. Remove neighboring rules, borders, gutters, and partial glyphs unless they belong to the selected artwork or facsimile panel. Keep semantic artwork crops separate from facsimile text-bearing panels.
